...
public class SeKillController implements InitializingBean {
     ...
     **/
    @ApiOperation("获取秒杀地址")
    @AccessLimit(second = 5, maxCount = 5, needLogin = true)   //限制访问次数，5秒内访问5次 注解版
...
//        }
    ...
    public RespBean doSecKill(@PathVariable String path, TUser user, Long goodsId)
    {
        ...
        System.out.println("----/{path}/doSeckill----");
        if (user == null) {
            return RespBean.error(RespBeanEnum.SESSION_ERROR);
        }
        ...
        //判断是否重复抢购
        String tSeckillOrder = (String) redisTemplate.opsForValue().get("order:" + user.getId() + ":" + goodsId);
        if (!StringUtils.isEmpty(tSeckillOrder)) {
            return RespBean.error(RespBeanEnum.REPEATE_ERROR);
        }

        //内存标记，减少Redis的访问,库存空了
...
//        return RespBean.success(tOrder);
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        list.forEach(goodsVo -> {
            ...
            EmptyStockMap.put(goodsVo.getId(), false);//有库存，用false
        });
    }
}
